...
const featuredCat = DataKucing.reduce((current, item) => (
  item.quantity > current.quantity ? item : current
), DataKucing[0]);
    ...
    >
      ...
      <div className="mt-8 rounded-lg border border-white/10 bg-white/5 p-4">
        ...
        <p className="mt-3 text-5xl font-black tabular-nums text-white">
          {featuredCat.quantity}
        </p>
      </div>
      <p className="mt-6 text-sm font-medium leading-6 text-amber-50/75">
        Tall cards are useful for featured profiles, hero items, and content that needs vertical focus.
      </p>
    </motion.article>
  );
}
